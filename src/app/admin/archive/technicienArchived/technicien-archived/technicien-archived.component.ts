...
@Component({
  selector: 'app-technicien-archived',
  templateUrl: './technicien-archived.component.html',
  styleUrl: './technicien-archived.component.css'
})
export class TechnicienArchivedComponent implements OnInit {
  technician: any;
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      this.contractService.getPageName = 'archived TECHNICIANS';
    });
  }
  searchText: string = '';
  ...
  getAllTechnician(): void {
    this.technicianService.getAllArchivedTechnicians().subscribe(technicians => {
  ...
  loadTechniciensortByrating(): void {
    if (this.sortByRating === 'RatingAsc') {
      this.technicianService.getByRatingAscarchived().subscribe(technicians => {
        this.technicians = technicians;
      });
    } else if (this.sortByRating === 'RatingDesc') {
      this.technicianService.getByRatingDescarchived().subscribe(technicians => {
        this.technicians = technicians;
      });
    }
  ...
  getBySpeciality(event: Event): void {
    ...
    if (poste === "ALL") {
      ...
      this.technicianService.getBySpecialityarchived(poste).subscribe(technicians => {
        this.technicians = technicians;
        if (this.technicians.length == 0) {
          this.listeVide == true;
        }
  ...
  loadTechniciens2(): void {
    ...
    if (this.startDateWork === 'startDateWorkAsc') {
      this.technicianService.getByStartDateWorkAscarchived().subscribe(technicians => {
        this.technicians = technicians;
      });
    } else if (this.startDateWork === 'startDateWorkDesc') {
      this.technicianService.getByStartDateWorkDescarchived().subscribe(technicians => {
        this.technicians = technicians;
      });
    }
  }
